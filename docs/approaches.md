...
*Companion doc: [`design.md`](design.md) covers the chosen architecture, the five
evaluation queries, and results.*

We evaluated five approaches (a global-embedding baseline + four ways to beat it),
adversarially red-teaming each on the five target queries and on the grading axes,
before committing. Below: what each is, when it's the right call, and why we chose
a hybrid.
...
### B. Structure-guided region binding (inference-time) ★ core of our choice
Decompose the query into (colour, garment) clauses; embed each garment region;
score `cos(garment_text, region) × colour_gate`; bind clauses to **distinct**
...
- **Weak when:** SigLIP is pooled-to-pooled (no true per-token late interaction),
  so the multivector index buys 5–15× storage for ~no recall gain — "theater."
  We kept its *lesson* (per-clause MAX over regions) and dropped its *index*.
...
- **Weak when:** local-1M-scale (per-image VLM captioning forces an API/GPU exactly
  ...
  exact on colour. We harvest its strength (scene/style) as an **optional** top-N
...
We validated the design against a survey of mid-2026 SOTA (general + fashion
...
- **Our region-binding rerank IS the current training-free SOTA family.** The
  decompose→per-clause-bind→conjunctive-aggregate pattern matches ComCLIP and
  ABE-CLIP (2025); our Hungarian distinct-region assignment independently
  reproduces the winning mechanism of TTM (Oct 2025).
- **We deliberately do NOT replace it with an instruction-tuned multimodal
  ...
  MMEB rank ≠ better binding, and general encoders *regress in-domain*
  (SigLIP2 0.261 vs fashion-tuned 0.283 R@10).
- **We deliberately do NOT add a VLM reranker.** A cross-encoder VLM could raise
...
- **We deliberately do NOT fine-tune** (hard-negative / NegCLIP): that would put
