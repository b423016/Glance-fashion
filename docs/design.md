...
*Companion doc: [`approaches.md`](approaches.md) covers the alternatives considered,
the trade-off scorecard, SOTA positioning, and future work.*

---

## 1. The problem, and why a vanilla CLIP baseline is not enough

We want text→image retrieval over a fashion gallery that resolves **garment ×
colour × environment × vibe**, handles **compositional** queries, and works
**zero-shot** (no query→image training labels).
...
SigLIP scores ~4.5/100 on controlled colour-binding. We confirmed it on our own
...
ones (negative discrimination — see §4).

**Design principle:** keep a strong fashion encoder for cheap *recall*, but add a
**training-free compositional re-ranker** that makes attribute→object binding
...
Apache-2.0) — SOTA open fashion text→image, runs on Apple-Silicon MPS. Store:
...
The mask has two consumers — *padded bbox → embedding*, *eroded mask → colour* —
...
query ─► parse ─► { clauses:[(red,tie),(white,shirt)], scene, formality }
        ...
        └─ Stage 2  RERANK (top-N only):
             per clause  s = MAX over regions of  cos(garment_text, region) × colour_gate
             Hungarian assignment (distinct regions)   ← clauses can't share a garment
             soft-MIN across clauses (α·min + (1−α)·mean)   ← require ALL clauses
...
degrade gracefully). `mode="global"` disables Stage 2 → the ablation baseline.
...
| 2 | *business attire in a modern office* | no bindable attribute → scene(office)+formality(formal) over the global embedding |
| 3 | *blue shirt on a park bench* | clause (blue, shirt) via region MAX + colour gate; scene(park) boost |
| 4 | *casual weekend, city walk* | no clause → scene(street)+formality(casual) + global vibe |
...
**global (vanilla fashion-CLIP) vs full (our pipeline), 4,113-image gallery:**

| Query | positives | P@8 g→f | nDCG@10 g→f | AP@20 g→f |
|-------|:---:|:---:|:---:|:---:|
| Q1 yellow raincoat | 11 | 0.38 → **0.62** | 0.47 → **0.68** | 0.26 → **0.54** |
| Q3 blue shirt (+park) | 245 | 0.50 → **0.62** | 0.51 → 0.47 | 0.19 → **0.28** |
| **Q5 red tie + white shirt** | 51 | **0.25 → 0.62** | 0.32 → **0.55** | 0.13 → **0.35** |
| S2 blue top | 245 | 0.75 → **1.00** | 0.72 → **0.93** | 0.44 → **0.59** |
| S3 red dress | 122 | 1.00 → 1.00 | 0.93 → **1.00** | 0.91 → **1.00** |
| S4 white top + black pants | 894 | 0.75 → **1.00** | 0.85 → **0.93** | 0.56 → **0.76** |
| S1 yellow jacket / S5 black jacket+white top | 11 / 778 | 0.88 / 0.50 (flat) | — | — |

**Mean over judgeable queries: P@8 0.63 → 0.78 (+25%), nDCG@10 +15%, AP@20 +35%.**
...
colour filtering; to prove we *bind* attributes we run colour-swap pairs and report
...
(P@8 0.75 → 1.00), and the *"red tie + white shirt"* board now surfaces genuine
...
binding-only own-P@8 here (0.25) is below its full-query 0.62 in §4's table, which
...
- **Zero-shot:** every scored signal is an embedding (garment text ↔ region) or a
...
**evaluation** (`evaluation/`), and the **data** (`data/`, `index/`). Ready to
`git init` + push whenever desired.
